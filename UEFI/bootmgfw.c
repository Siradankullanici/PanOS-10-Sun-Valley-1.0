...
void dramatic_sound_effects(EFI_SYSTEM_TABLE* SystemTable, UINTN Columns, UINTN Rows, UINTN effectDuration) {
    ...
    for (UINTN i = 0; i < iterations; i++) {
        ...
        Print(L"PAN OS 10 SUN VALLEY IS HERE!");
...
EFI_STATUS EFIAPI efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable) {
    ...
    complex_blur(SystemTable, L"PAN OS 10 SUN VALLEY HERE!", 10, 10);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);

    pixel_rain(SystemTable, Columns, Rows);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);

    wave_animation(SystemTable, L"PAN OS 10 SUN VALLEY HERE!", 0, 10, 2);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);

    text_explosion(SystemTable, L"PAN OS 10 SUN VALLEY HERE!", Columns / 2, Rows / 2);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);

    fading_text(SystemTable, L"PAN OS 10 SUN VALLEY HERE!", 5, 5);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);
    ...
    flickering_text(SystemTable, L"ANTIVIRUSDEFENDER10 HERE!", 20, 10);
    dramatic_sound_effects(SystemTable, Columns, Rows, effectDuration);
